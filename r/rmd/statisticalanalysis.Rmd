...
*Group 3: Select band B11 because the band presents the lowest  Inter-class Correlation.
...
The evaluation of the clustering uses distinct cluster metrics to define the quality of each model. Also, we include in the analysis the model's accuracy considering the labeled target. To calculate the accuracy, we define a label for each cluster based on the majority, where the cluster label will be the class with more members in the cluster. We formulated a voting schema through the metrics where each metric contributes with one single vote. The vote of the model depends on how the metric works. In some metrics, the target is to select the maximum between all band combinations by the given values; in others, the objective is to select the minimum value. In the code below, we calculate distinct cluster metrics to evaluate the quality of each model and voting.  
...
for(i in 1:length(best_clusters)){
  ...
  train_test_eval2 <- append(train_test_eval2, list(pred))
}
```  

```{r}

train_test_eval3 <- list()
...
for(i in 1:length(best_clusters)){
  ...
  train_test_eval3 <- append(train_test_eval3, list(pred))
}
```
...
best_clusters_list <- readRDS("../../data/rds/best_clusters_list.rds")
acc2_tiles <- accuracy_analysis(samples, best_clusters_list,
                               "2018-07-28","2019-07-12", "../../data/csv/B04-B07-B12-NDVI-EVI")
acc2_tiles
```
